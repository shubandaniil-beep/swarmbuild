...
def _user_from_header_or_query(db: Session, authorization: str, token: str | None,
                               session_cookie: str = "") -> User:
    """Authenticate downloads via HttpOnly cookie, Authorization, or legacy ?token=."""
    raw = ""
...
def artifact_download(project_id: str, artifact_id: str,
                      ...
                      db: Session = Depends(get_db)):
    user = _user_from_header_or_query(db, authorization, token, sb_session)
    _accessible_project(db, project_id, user)
    a, f = _artifact_file(db, project_id, artifact_id)
    if user.role != "admin" and a.artifact_type != "final":
        raise HTTPException(404, "artifact not found")
