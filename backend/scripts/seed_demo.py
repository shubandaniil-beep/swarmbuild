...
def main() -> None:
    ...
    db = SessionLocal()
    try:
        # Budgets below sum to ~$500 → ~50k credits; grant headroom so the
        # showcase runs finish instead of freezing in needs_topup.
        demo_credits = 100_000
        demo = db.query(User).filter(User.email == "demo@swarmbuild.ai").first()
        if not demo:
            demo = User(email="demo@swarmbuild.ai",
                        password_hash=hash_password("demo12345"),
                        token_balance=demo_credits,
                        lifetime_tokens_granted=demo_credits)
            db.add(demo)
            db.commit()
        elif (demo.token_balance or 0) < demo_credits:
            demo.token_balance = demo_credits
            db.commit()
