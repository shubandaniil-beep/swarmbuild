...
const INPUT = "bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-sm outline-none focus:border-amber-400";
    ...
    <div className="space-y-8">
      ...
      {(error || message) && (
        <div className={`rounded-lg border px-4 py-3 text-sm ${
          error ? "border-red-500/30 bg-red-500/10 text-red-300" : "border-amber-500/30 bg-amber-500/10 text-amber-200"
        }`}>
          {error || message}
        </div>
      )}
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_380px]">
        ...
        <aside className="space-y-4">
          <form onSubmit={create} className="card p-5 space-y-3">
            ...
            <button className="w-full rounded-lg bg-amber-400 px-4 py-2 text-sm font-semibold text-zinc-950 hover:bg-amber-300">
              Добавить
            </button>
          </form>
