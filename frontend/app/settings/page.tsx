        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {summary.packs.map((pack) => (
            <button key={pack.id}
                    ...
                    disabled={!!busy}
                    className="card p-4 text-left hover:border-indigo-400/50 disabled:opacity-50">
              <span className="block text-sm font-semibold">{pack.label}</span>
              <span className="mt-2 block text-2xl font-bold text-indigo-400">
              ...
              <span className="mt-4 inline-flex text-xs text-indigo-300">
              ...
              <tbody>
                {topups.map((t) => (
                  <tr key={t.id} className="border-b border-zinc-900">
                    ...
                    <td>${t.amount_usd}</td>
                    <td>{t.provider}</td>
                    <td className={t.status === "paid" ? "text-green-400" : "text-indigo-300"}>
                      {t.status}
                    </td>
                    <td>
                      {t.payment_url ? (
                        <a href={t.payment_url} className="text-indigo-300">открыть</a>
                      ) : (
