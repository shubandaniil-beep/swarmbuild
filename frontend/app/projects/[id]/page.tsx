...
const NEXT_IMPROVEMENTS = [
  {
    ...
    credits: 800,
    icon: "rocket",
    detail: "План запуска и понятные шаги публикации",
  },
  {
    id: "domain-setup",
    ...
    credits: 700,
    icon: "globe",
    detail: "Адрес сайта и проверки открытия",
  },
  {
    id: "telegram-integration",
    title: "Добавить Telegram-бота",
    action: "Telegram integration",
    credits: 1500,
    icon: "send",
    detail: "Бот с командами и базовой логикой",
  },
  {
    id: "pitch-deck",
    title: "Сделать pitch deck",
    action: "Pitch deck",
    credits: 1200,
    icon: "presentation",
    detail: "Структура слайдов для клиента или инвестора",
  },
  {
    id: "staff-guide",
    title: "Сделать инструкцию для сотрудников",
    action: "Employee instruction",
    credits: 1200,
    icon: "clipboardCheck",
    detail: "Понятный регламент использования результата",
  },
];
...
const TYPE_LABELS: Record<string, string> = {
  ...
  code_project: "кодовый проект",
  document_project: "документ",
  business_project: "бизнес-пакет",
  presentation_project: "презентация",
  research_project: "исследование",
  ...
  automation_script: "автоматизация",
  python_utility: "утилита",
  saas_mvp: "SaaS MVP",
  mobile_app_concept: "концепт приложения",
  education_project: "учебный проект",
  diploma_or_coursework: "диплом / курсовая",
  pitch_deck: "pitch deck",
  marketing_pack: "маркетинг-пакет",
  custom: "свой формат",
};
...
const OUTPUT_LABELS: Record<string, string> = {
  ...
  technical_spec: "спецификация",
  marketing_plan: "маркетинг-план",
  financial_model: "финмодель",
  deployment_guide: "инструкция запуска",
  user_manual: "инструкция пользователя",
  roadmap: "roadmap",
  branding_copy: "тексты / брендинг",
};
...
const STATUS_LABELS: Record<string, string> = {
  ...
  ready: "готов",
  partial_ready: "готов с ограничениями",
  failed: "ошибка",
  needs_topup: "нужно пополнить кредиты",
  needs_provider: "AI-модели временно недоступны",
  cancelled: "отменён",
};

function label(map: Record<string, string>, value?: string | null) {
  if (!value) return "авто";
...
function friendlyLimitations(text: string) {
  return text
    .replace(/^# .*\n+/, "")
    .replaceAll("This is an MVP skeleton, not a production system.", "Это MVP-версия, а не полностью продакшн-система.")
    .replaceAll("No authentication and minimal error handling.", "Авторизация и обработка ошибок пока минимальные.")
    .replaceAll("No automated test suite beyond smoke checks.", "Автотесты ограничены базовыми smoke-проверками.")
    .replaceAll("Open issue", "Открытое замечание");
}
...
function ProjectContent({ id }: { id: string }) {
  ...
  if (!project) return <PageLoader label="Загружаю проект…" />;
  ...
  const summary = done
    ? `Готов финальный пакет: ${finals.length} файлов, потрачено ${spentCredits} credits.` +
      (project.status === "partial_ready"
        ? " Результат готов частично — ниже указаны ограничения."
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="mt-2 flex flex-wrap gap-2 text-xs">
          {projectTags.map((t) => (
            <span key={t} className="px-2 py-1 rounded-full bg-zinc-800/80 text-zinc-400">{t}</span>
          ))}
          {project.requested_outputs?.map((o) => (
            <span key={o} className="px-2 py-1 rounded-full bg-amber-400/10 text-amber-300">
        ...
        {project.parent_project_id && (
          <p className="mt-2 text-sm text-zinc-500">
            ↳ доработка проекта{" "}
            <Link className="text-amber-300" href={`/projects/${project.parent_project_id}`}>
              #{project.parent_project_id.slice(0, 8)}
            </Link>
          </p>
        )}
      </div>
      ...
      {project.status === "needs_topup" && (
        <div className="rounded-xl border border-amber-800/60 bg-amber-950/40 p-4 text-sm text-amber-200">
          Кредиты закончились до завершения проекта. Собран частичный результат —
          пополните баланс и запустите доработку, чтобы продолжить.
        </div>
      )}
      ...
      {project.status === "needs_provider" && (
        <div className="rounded-xl border border-amber-800/60 bg-amber-950/30 p-4 text-sm text-amber-200 space-y-3">
          <p>
            AI-модели сейчас недоступны: чаще всего это временный лимит запросов
            у провайдера. Подождите минуту и запустите проект заново — прогресс
            по завершённым этапам сохраняется.
          </p>
          <button onClick={restartProject} disabled={!!busyAction}
                  className="btn-primary px-5 py-2 text-sm disabled:opacity-50">
            {busyAction === "restart" ? "Запускаю..." : "↻ Запустить заново"}
          </button>
        </div>
      )}
      ...
      {project.status === "failed" && (
        <div className="rounded-xl border border-red-900/60 bg-red-950/30 p-4 text-sm text-red-200 space-y-3">
          <p>
            Проект остановился с ошибкой. Подробности — в журнале ниже. Можно
            запустить его заново: завершённые этапы не выполняются повторно.
          </p>
          <button onClick={restartProject} disabled={!!busyAction}
                  className="btn-primary px-5 py-2 text-sm disabled:opacity-50">
            {busyAction === "restart" ? "Запускаю..." : "↻ Запустить заново"}
      ...
      {finals.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {finals.map((a) => (
              <ArtifactCard key={a.id} projectId={id} artifact={a} />
            ))}
          </div>
          <div className="mt-4 flex gap-3">
            <Link href={`/projects/${id}/artifacts`} className="btn-ghost px-5 py-2 text-sm">
              Все финальные файлы →
            </Link>
            {(done || project.status === "needs_topup") && (
              <a href={downloadUrl(`/api/projects/${id}/download`)}
                 className="btn-primary px-5 py-2 text-sm">
                <Icon name="download" size={14} /> project.zip
              </a>
            )}
          </div>
        </div>
      ...
      {limitations && (
        <div className="card p-5">
          ...
          <pre className="text-xs text-zinc-400 whitespace-pre-wrap leading-relaxed">
            {friendlyLimitations(limitations)}
          </pre>
        </div>
      )}
      ...
      {done && (
        <div>
          ...
          <p className="text-sm text-zinc-500 mb-4">
            Каждое действие запускает связанный проект-доработку с тем же контекстом.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {NEXT_IMPROVEMENTS.map((a) => (
              <button key={a.id} onClick={() => continueProject(a.action, a.credits, a.id)}
                      disabled={!!busyAction}
                      className="card p-4 text-left hover:border-amber-400/50 disabled:opacity-50 transition-colors">
                <span className="flex items-start gap-3">
                  <IconTile name={a.icon as IconName} size={32} />
                  <span className="min-w-0">
                    <span className="block text-sm font-medium text-zinc-100">
                      {busyAction === a.id ? "Запускаю..." : a.title}
                    </span>
                ...
                <span className="mt-3 inline-flex rounded-full border border-amber-400/30 bg-amber-400/10 px-2.5 py-1 text-xs font-medium text-amber-300">
                  {a.credits.toLocaleString("ru-RU")} credits
                </span>
              </button>
            ))}
          </div>
