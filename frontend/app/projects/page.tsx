...
const MODE_ICON: Record<string, string> = {
  code: "⌨️", document: "📄", business: "💼", mixed: "🧩", auto: "✨",
};
...
function ProjectsContent() {
  ...
  const spent = projects.reduce((s, p) => s + (p.budget?.spent_usd || 0), 0);
    ...
    <div className="space-y-8">
      ...
      {projects.length > 0 && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard value={`$${spent.toFixed(4)}`} label="использовано" />
        </div>
      )}
        ...
        {FILTERS.map((f) => (
          <button key={f.key} onClick={() => setFilter(f.key)}
                  className={`text-xs px-3.5 py-1.5 rounded-full border transition-all ${
                    filter === f.key
                      ? "border-indigo-400/70 text-indigo-300 bg-indigo-400/10"
      ...
      {projects.length === 0 ? (
        <EmptyState icon="🐝" title="Первая задача ещё не создана"
          ...
          {shown.map((p) => (
            <div key={p.project_id} className="card card-hover p-5 flex flex-col">
              <div className="flex items-center gap-2.5">
                <span className="text-lg">{MODE_ICON[p.project_mode] || "✨"}</span>
                <Link href={`/projects/${p.project_id}`}
                      className="font-semibold truncate hover:text-indigo-300 transition-colors">
                  {p.title}
                </Link>
              ...
              <div className="mt-4 flex items-center gap-2 text-xs flex-wrap">
                <span className="chip chip-indigo">${p.budget_usd}</span>
                ...
                <span className="ml-auto flex gap-2">
                  ...
                  {(p.status === "ready" || p.status === "partial_ready") && (
                    <a href={downloadUrl(`/api/projects/${p.project_id}/download`)}
                       className="btn-ghost px-3 py-1.5">⬇ ZIP</a>
                  )}
                </span>
              </div>
            </div>
