...
import { api, getToken, getUser, setAuth } from "@/lib/api";
...
const RISK_LABEL: Record<string, { text: string; cls: string }> = {
  ...
  medium: { text: "средний", cls: "text-indigo-300" },
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (typeof res.token_balance === "number") {
        const user = getUser();
        if (user) setAuth(getToken(), { ...user, token_balance: res.token_balance });
      }
          ...
          {MODES.map((m) => (
            <button key={m.key} type="button" onClick={() => setMode(m.key)}
                    className={`text-left px-4 py-3 rounded-xl border transition-all duration-200 ${
                      mode === m.key
                        ? "border-indigo-400/70 bg-indigo-400/5 shadow-[0_0_16px_rgba(99, 102, 241,0.12)]"
                        : "border-zinc-800 hover:border-zinc-600 bg-zinc-900/30"}`}>
              <span className={`font-medium text-sm ${mode === m.key ? "text-indigo-300" : ""}`}>
                {m.label}
              </span>
              ...
              <button key={m.key} type="button" onClick={() => setPersonality(m.key)}
                      className={`text-left px-4 py-3 rounded-xl border transition-all duration-200 ${
                        personality === m.key
                          ? "border-indigo-400/70 bg-indigo-400/5 shadow-[0_0_16px_rgba(99, 102, 241,0.12)]"
                          : "border-zinc-800 hover:border-zinc-600 bg-zinc-900/30"}`}>
                <span className={`font-medium text-sm ${personality === m.key ? "text-indigo-300" : ""}`}>
                  {m.display_name}
                </span>
                <span className="block text-xs text-zinc-500 mt-0.5">{m.hint}</span>
              </button>
            ...
            <button key={b.value} type="button"
                    ...
                    className={`px-4 py-2.5 rounded-xl border text-sm transition-all duration-200 ${
                      !custom && budget === b.value
                        ? "border-indigo-400/70 text-indigo-300 bg-indigo-400/5 shadow-[0_0_16px_rgba(99, 102, 241,0.12)]"
