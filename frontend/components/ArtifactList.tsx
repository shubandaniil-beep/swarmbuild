      ...
      <ul className="space-y-1 text-sm max-h-[70vh] overflow-y-auto log-scroll">
        {artifacts.map((a) => (
          <li key={a.id}>
            <button
              onClick={() => open(a)}
              className={`text-left w-full px-2 py-1.5 rounded-lg font-mono text-[13px] hover:bg-zinc-900 transition-colors ${
                selected === a.id ? "text-amber-300" : "text-zinc-300"
              }`}
            >
              <Icon name={a.display_name.endsWith(".zip") ? "package" : "fileText"}
                    size={13} className="inline shrink-0 -mt-0.5 mr-1.5 opacity-70" />
              {a.display_name}
            </button>
          </li>
        ))}
